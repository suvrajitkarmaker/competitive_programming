...
struct line
{
    ...
    int doSegIntersectSeg(line v)
    {
        ...
        if((d1^d2)==-2 && (d3^d4)==-2)
            return 2;//intersects;
        if(!d1&&compare((v.a - a).dot(v.a - b))<=0)
            return 1; //intersect strictly inside
        if(!d2&&compare((v.b - a).dot(v.b - b))<=0)
            return 1;
        if(!d3&&compare((a - v.a).dot(a - v.b))<=0)
            return 1;
        if(!d4&&compare((b - v.a).dot(b - v.b))<=0)
            return 1;
...
struct polygon
{
    ...
    int doesLineIntersectPolygon(line u)
    {
        ...
        for(int i=0; i<n; i++)
        {
            if(l[i].doSegIntersectSeg(u)==2)
                return 1;//some intersection
            if(l[i].doSegIntersectSeg(u)==1)
                k=1;
        }
        if(pointInPolygon(u.a)||pointInPolygon(u.b))
            return 1;
        if(!k)
            return 0;//no intersection
...
int main()
{
    ...
    for(int ii=0; ii<te; ii++)
    {
        ...
        for(int i=0; i<n; i++)
        {
            for(int j=i+1; j<n; j++)
            {
                if(li[i].doSegIntersectSeg(li[j]))
                    seg+=2;
            }
